...
public class AdminAccountService {
    ...
    public AdminAccountDtos.AdminSummary create(AdminAccountDtos.CreateAdminRequest req) {
        ...
        if (!autoGenerated && req.getPassword().length() < 8) {
            throw ApiException.badRequest("Password must be at least 8 characters");
        }
        String password = autoGenerated ? generateTempPassword() : req.getPassword();
        ...
        // Only put the password in the email when we're the one who made it up - if the Super
        // Admin chose it themselves, echoing it back in cleartext over email would just be an
        // unnecessary leak of a password they already know and may be sharing out of band.
        if (autoGenerated) {
            ...
            emailService.send(admin.getEmail(), "Your City Apartments Portal admin account",
                    "An admin account has been created for you. Log in with the password your administrator gave you.");
        }
